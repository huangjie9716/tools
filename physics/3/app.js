// ================================================================
// app.js — 全局状态管理、页面导航、冷却控制（调试模式：冷却失效）、主界面更新、重置功能
...
function loadState() {
    const s = loadJSON('formulaGameState_v4');
    if (s) {
...
function saveState() {
    saveJSON('formulaGameState_v4', G);
}

let G = loadState();

// ----- 公共工具（shuffleArray / renderKatex / renderDisplayKatex 已迁移至 utils.js）-----
...
// ================================================================
export {
    G, saveState, showPage, playSfx, updateMainUI, enterMain, enterCompete,
    getTotalCards, getCollectedCards, getTitleBadge, getStarIcon,
    getCurrentTitle, getTitleCongrats, showGameModal, closeGameModal, confirmGameModal,
    showTitlePopup, showUnlockPopup,
    isInCooldown, getCooldownRemaining, startCooldown, showCooldown, closeCooldown,
    renderKatex, renderDisplayKatex, shuffleArray
};

// ================================================================
// 暴露全局（兼容内联 onclick 与旧式调用，保证游戏可用）
// ================================================================
window.G = G;
