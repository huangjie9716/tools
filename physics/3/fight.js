...
import { G, saveState, showPage, showGameModal, playSfx, updateMainUI, getStarIcon } from './app.js';
import { shuffleArray, loadJSON, saveJSON } from './utils.js';
import { markOptions } from './quiz-utils.js';
import { FIGHT_DATA } from './fight-data.js';
...
function loadFightPractice() {
    fightPracticeCount = loadJSON('fightPractice_v1', {});
}

function saveFightPractice() {
    saveJSON('fightPractice_v1', fightPracticeCount);
}

// ----- 进入百草园 -----
function enterFight() {
    loadFightPractice();
    const allData = FIGHT_DATA || [];
    if (allData.length === 0) {
...
function selectFightOption(optIdx) {
    ...
    markOptions(document.querySelectorAll('.fight-opt'), { answer: q.answer, selected: optIdx, correctClass: 'correct', wrongClass: 'wrong', selectedCorrectClass: 'correct' });
...
// ================================================================
export { enterFight, selectFightOption };
