...
function renderAdventureMap() {
    ...
    // 关卡地图时页面标题保持“演武场”
    const mapTitleEl = document.getElementById('levelPageTitle');
    if (mapTitleEl) mapTitleEl.textContent = '演武场';
...
function renderLevelQuestion() {
    ...
    const container = document.getElementById('levelContent');
    // 关卡名与“演武场”合并，显示在页面标题右侧（如“演武场 · 竹林试炼”），答题区内不再单独显示标签
    const levelNames = ['竹林试炼', '武林擂台', '华山之巅'];
    const levelName = `演武场 · ${levelNames[G.currentLevel - 1] || '第' + G.currentLevel + '关'}`;
    const quizTitleEl = document.getElementById('levelPageTitle');
    if (quizTitleEl) quizTitleEl.textContent = levelName;
...
// 暴露全局
window.LEVEL_QUESTIONS = LEVEL_QUESTIONS;
window.isLevelInQuiz = () => levelInQuiz;
