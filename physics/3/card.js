...
import { G, saveState, showPage, showGameModal, showTitlePopup, showUnlockPopup, playSfx, updateMainUI, getTotalCards, getCollectedCards, getCurrentTitle, getTitleCongrats, isInCooldown, showCooldown, getCooldownRemaining, startCooldown } from './app.js';
import { renderKatex, renderDisplayKatex, shuffleArray } from './utils.js';
import { markOptions, ICON_CORRECT, ICON_WRONG } from './quiz-utils.js';
...
function selectQuizOption(idx) {
    ...
    markOptions(document.querySelectorAll('#quizOptions .opt'), { answer: q.ans, selected: idx });
    ...
    if (idx === q.ans) {
        ...
        fb.innerHTML = `<span class="fb-correct-icon">${ICON_CORRECT}</span> 回答正确！`;
        playSfx('correct');
        setTimeout(() => {
        ...
        fb.innerHTML = `<span class="fb-error-icon">${ICON_WRONG}</span> 心法参悟有误，走火入魔！`;
        playSfx('wrong');
...
// ================================================================
export {
    showCardCollection, renderCardGrid, openCardModal, closeCardModal,
    startFullCardQuiz, startFragmentQuiz, selectQuizOption,
    collectSingleCard, updateCardStatus
};
